...
{
    // Convert bithereum:// to bithereum:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("bithereum://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "bithereum:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Bithereum.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for Bithereum.lnk
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a bithereum.desktop file to the autostart directory:
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the bithereum app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add bithereum app to startup item list
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Bithereum-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  bithereum-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("Bithereum-Qt"));
