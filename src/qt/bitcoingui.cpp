...
{
    resize(900, 520);
    setWindowTitle(tr("Bithereum") + " - " + tr("Wallet"));
#ifndef Q_OS_MAC
...
#endif
    setObjectName("bithereum");
    setStyleSheet("#bithereum { background-color: qradialgradient(cx: -0.8, cy: 0, fx: -0.8, fy: 0, radius: 1.4, stop: 0 #dedede, stop: 1 #efefef);  }");
    // Accept D&D of URIs
    setAcceptDrops(true);
...
void BitcoinGUI::createActions()
{
    ...
    aboutAction = new QAction(tr("&About Bithereum"), this);
...
void BitcoinGUI::setClientModel(ClientModel *clientModel)
{
    ...
    if(clientModel)
    {
        ...
        if(clientModel->isTestNet())
        {
            ...
            if(trayIcon)
            {
                trayIcon->setToolTip(tr("Bithereum client") + QString(" ") + tr("[testnet]"));
...
void BitcoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
        ...
        if (nValidUrisFound)
            ...
            notificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid Bithereum address or malformed URI parameters."));
    }
