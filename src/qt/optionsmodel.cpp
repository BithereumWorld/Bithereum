...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeBithereumAmount"))
        settings.setValue("nAnonymizeBithereumAmount", 1000);
    nAnonymizeBithereumAmount = settings.value("nAnonymizeBithereumAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeBithereumAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeBithereumAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeBithereumAmount:
            return QVariant(nAnonymizeBithereumAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeBithereumAmount:
            nAnonymizeBithereumAmount = value.toInt();
            settings.setValue("nAnonymizeBithereumAmount", nAnonymizeBithereumAmount);
            emit AnonymizeBithereumAmountChanged(nAnonymizeBithereumAmount);
            break;
        default:
            break;
        }
