...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeBithereumAmount = BitcoinUnits::formatWithUnit(nDisplayUnit, nAnonymizeBithereumAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeBithereumAmount = strAnonymizeBithereumAmount.remove(strAnonymizeBithereumAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeBithereumAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeBithereumAmount*COIN) nMaxToAnonymize = nAnonymizeBithereumAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeBithereumAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeBithereumAmount));
        strAnonymizeBithereumAmount = strAnonymizeBithereumAmount.remove(strAnonymizeBithereumAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeBithereumAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
...
void OverviewPage::toggleDarksend(){
    ...
    if(!fEnableDarksend){
        ...
        if(nAnonymizeBithereumAmount == 0){
