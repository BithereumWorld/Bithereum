...
void Shutdown()
{
    ...
    RenameThread("bithereum-shutoff");
...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: bithereum.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: bithereumd.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizebithereumamount=<n> " + _("Keep N Bithereum anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Bithereum is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Bithereum is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Bithereum version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    nAnonymizeBithereumAmount = GetArg("-anonymizebithereumamount", 0);
    if(nAnonymizeBithereumAmount > 999999) nAnonymizeBithereumAmount = 999999;
    if(nAnonymizeBithereumAmount < 2) nAnonymizeBithereumAmount = 2;
    ...
    LogPrintf("Anonymize Bithereum Amount %d\n", nAnonymizeBithereumAmount);
